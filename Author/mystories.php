...
<body>
    <?php
        ...
        $sql=$db->prepare(" select count(*) from story where uid = '$uid'");
        $sql->execute();
        $row=$sql->fetch();
        $totalresults=$row[0]; 
        ...
        $sql=$db->prepare("select * from story where uid ='$uid'  ORDER BY id DESC  limit ".$limitnumber.",".$NoOfResults);
        $sql->execute();
        $row=$sql->fetch();
        $count=$sql->rowCount();
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>You are about to delete this Story, this procedure is irreversible.</p>
            ...
            <?php for($rec=0;$rec<$count;$rec++){
                ...
                <div class="col-lg-4 col-sm-6 portfolio-item">
                    <div class="card h-100">
                        <a href="<?php echo "displaystory.php?st=".$row["id"]; ?>"><img class="card-img-top" width="300" height="300" src="<?php if($row["cover"]==""){echo "http://placehold.it/700x400";} else echo "uploads/".$row["cover"]?>" alt="http://placehold.it/700x400"></a>
                            <div class="card-body">
                                <h4 class="card-title" align="center">
                                    <a href="<?php echo "displaystory.php?st=".$row["id"]; ?>"><?php echo $row['title'] ;?></a>
                                
                                <hr>
                                    <a href="Edit.php?id=<?php echo $row["id"];?>&ty=st" class="btn btn-warning" role="button">Edit</a>
                                    
                                    <a href="#" data-href="Delete.php?id=<?php echo $row["id"];?>&type=st" data-toggle="modal" data-target="#confirm-delete" class="btn btn-danger" role="button">Delete</a>
